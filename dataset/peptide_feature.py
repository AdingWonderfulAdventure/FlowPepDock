...
_AA2TIP_IDX_TENSOR_CACHE = {}
_BACKBONE_ATOM_IDS_CACHE = {}
_NAN_ROW_CACHE = {}


def _get_cached_aa2tip_idx(device):
    key = str(device)
    tensor = _AA2TIP_IDX_TENSOR_CACHE.get(key)
    if tensor is None:
        tensor = torch.tensor(aa2tip_idx, device=device)
        _AA2TIP_IDX_TENSOR_CACHE[key] = tensor
    return tensor


def _get_cached_backbone_atom_ids(device):
    key = str(device)
    tensor = _BACKBONE_ATOM_IDS_CACHE.get(key)
    if tensor is None:
        tensor = torch.tensor([0, 2, 3], device=device)
        _BACKBONE_ATOM_IDS_CACHE[key] = tensor
    return tensor


def _get_cached_nan_row(device, dtype):
    key = (str(device), str(dtype))
    tensor = _NAN_ROW_CACHE.get(key)
    if tensor is None:
        tensor = torch.full((1, 3), float('nan'), device=device, dtype=dtype)
        _NAN_ROW_CACHE[key] = tensor
    return tensor
...
def _read_pdb_for_atom_features(pep_noh):
    try:
        return read_pdb_with_seq(pep_noh)
    except Exception:
...
def lig_atom_featurizer(pep_noh):
    ...
    mol = _read_pdb_for_atom_features(pep_noh)
...
def get_ori_peptide_feature_mda(peptide_file, lm_embedding_chains=None, match = False, modify = None):    
    ...
    # Build peptide atom bonds from the actual postprocessed PDB rather than a
    # full residue template. This keeps edge indices aligned with atom lists
    # when some sidechain heavy atoms are missing; complete peptides remain
    # unchanged because read_pdb_with_seq reconstructs the same connectivity.
    all_edge_index = get_edges_from_pdb_with_seq(peptide_file)
    ...
    with torch.no_grad():
        ...
        for i,chain in enumerate(peptide_mol.segments):
            ...
            # 取“原始坐标”（peptide_ori）时，必须按 residue id 对齐，而不是用 idx 硬怼；
            # 不然一旦 residue 排序/缺失原子，就会出现空选择然后 index 越界（你看到的 size 0 就是这玩意）。
            atoms = []
            ori_res_map = {}
            for res_idx_ori, residue in enumerate(peptide_ori.segments[i].residues):
                key = int(residue.resid) if residue.icode == "" else f"{residue.resid}{residue.icode.strip()}"
                ori_res_map[key] = residue

            for mol_residue in chain_pure_res_lis:
                key = int(mol_residue.resid) if mol_residue.icode == "" else f"{mol_residue.resid}{mol_residue.icode.strip()}"
                if key not in ori_res_map:
                    raise RuntimeError(f"{peptide_file}: peptide_ori 缺少 residue {key}，无法对齐原子坐标")
                ori_residue = ori_res_map[key]
                ori_atoms = ori_residue.atoms
                if len(ori_atoms) > 0:
                    mask = (ori_atoms.altLocs == "") | (ori_atoms.altLocs == "A")
                    ori_atoms = ori_atoms[mask]
                for atom_name in mol_residue.atoms.names:
                    sel = ori_atoms.select_atoms(f"name {atom_name}")
                    if len(sel) == 0:
                        raise RuntimeError(
                            f"{peptide_file}: residue {key} 原子选择失败 name={atom_name}（匹配到 0 个）"
                        )
                    # 可能遇到 altloc 或重复记录：这里取第一个，保证不断流；更细的选择策略后面再说
                    atoms.append(sel[0])
...
def get_updated_peptide_feature(data, device, top_k=24):
    ...
    atom2res_index_full = data['pep_a'].atom2res_index + data['pep'].ptr[data['pep_a'].batch]
    ...
    tip_target = _get_cached_aa2tip_idx(device)[data['pep_a'].atom2resid_index]
    tip_mask = atom2atomid_index == tip_target
    tips = ca.clone()
    if tip_mask.any():
        tip_res_idx = atom2res_index_full[tip_mask]
        tips.index_copy_(0, tip_res_idx, pep_a_pos[tip_mask])
    
    s,t = torch_cluster.radius_graph(data['pep_a'].atom2res_index.to(torch.float), 1, batch=data['pep_a'].batch,max_num_neighbors=10000)
    index = atom2res_index_full[t]
    ...
    row_ids = torch.arange(len(index), device=device)
    zero_idx = torch.zeros((), dtype=row_ids.dtype, device=device)
    nan_row = _get_cached_nan_row(device, pep_a_pos.dtype)
    mask = torch.where(torch.isin(index -1, index), row_ids, zero_idx)
    c_before = torch.cat([nan_row, c])[mask]
    mask = torch.where(torch.isin(index +1, index), row_ids + 2, zero_idx)
    n_after = torch.cat([nan_row, n])[mask]
    ca_after = torch.cat([nan_row, ca])[mask]

    n_c_before = _normalize(torch.where(torch.logical_and(1.3 < (n - c_before).norm(dim=-1), (n - c_before).norm(dim=-1) < 1.4).unsqueeze(-1).repeat(1,3), (n - c_before), nan_row))
    ca_n = _normalize(torch.where(torch.logical_and(1.4 < (ca - n).norm(dim=-1), (ca - n).norm(dim=-1) < 1.5).unsqueeze(-1).repeat(1,3), (ca - n), nan_row))
    c_ca = _normalize(torch.where(torch.logical_and(1.5 < (c-ca).norm(dim=-1), (c-ca).norm(dim=-1) < 1.6).unsqueeze(-1).repeat(1,3), (c-ca), nan_row))
    n_after_c = _normalize(torch.where(torch.logical_and(1.3 < (n_after-c).norm(dim=-1), (n_after-c).norm(dim=-1) < 1.4).unsqueeze(-1).repeat(1,3), (n_after-c), nan_row))
    ca_after_n_after = _normalize(torch.where(torch.logical_and(1.4 < (ca_after-n_after).norm(dim=-1), (ca_after-n_after).norm(dim=-1) < 1.5).unsqueeze(-1).repeat(1,3), (ca_after-n_after), nan_row))
    ...
    center_pos.index_add_(0, index=atom2res_index_full, source=pep_a_pos)
    center_pos = center_pos / torch.bincount(atom2res_index_full).unsqueeze(1)

    center_sidechain_pos = torch.zeros((len(data['pep'].x), 3)).to(device)
    sidechain_mask = ~torch.isin(atom2atomid_index, _get_cached_backbone_atom_ids(device))
    if sidechain_mask.any():
        sidechain_index = atom2res_index_full[sidechain_mask]
        center_sidechain_pos.index_add_(0, index=sidechain_index, source=pep_a_pos[sidechain_mask])
        sidechain_count_raw = torch.bincount(sidechain_index, minlength=len(data['pep'].x)).unsqueeze(1)
        sidechain_count = sidechain_count_raw.clamp_min(1)
        center_sidechain_pos = center_sidechain_pos / sidechain_count
        center_sidechain_pos[sidechain_count_raw.squeeze(1) == 0] = ca[sidechain_count_raw.squeeze(1) == 0]
    else:
        center_sidechain_pos = ca.clone()
    ...
    # edge_connect 只该表达“是否相邻残基且几何可用”，必须保持旋转不变；
    # 原实现用 `!= 0` 按分量判断会把全局坐标轴掺进来，训练后会破坏等变性。
    valid_res = torch.isfinite(n_c_before).all(dim=-1)
    edge_connect = (
        (torch.abs(edge_index[0] - edge_index[1]) == 1)
        & valid_res[torch.max(edge_index, dim=0)[0]]
    ).long()
