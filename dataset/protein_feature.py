...
# 受体也复用“肽 onehot 词表”（104 维）：
# - 允许非标准“氨基酸残基”（你给的支持表那一批）
# - 蛋白但不在支持表：统一映射到 'X'（onehot 第104维兜底）
# - 非蛋白配体/辅因子：通过 PDB record_type=HETATM 剔除（避免 SAM 这类碰瓷）
from dataset.peptide_feature import three2idx, three2self, aa2tip  # noqa: E402
RES_MAX_NATOMS = 24
...
def obatin_edge(res_lis, src, dst):
    ...
    dst_idx = int(dst.max()) if hasattr(dst, "max") else int(dst)
    return dist.min() * 0.1, dst_idx * 0.1
...
def _pairwise_row_distance(points: np.ndarray, edge_index: torch.Tensor) -> torch.Tensor:
    edge_src = edge_index[0].detach().cpu().numpy()
    edge_dst = edge_index[1].detach().cpu().numpy()
    deltas = points[edge_src] - points[edge_dst]
    return torch.from_numpy(np.linalg.norm(deltas, axis=1))
    ...
    with torch.no_grad():
        ...
        for i, chain in enumerate(protein_mol.segments):
            ...
            for res_idx, residue in enumerate(chain.residues):
                ...
                # 只接受 ATOM 记录（聚合物蛋白/肽）：
                # - 非蛋白 HET（SAM/GOL/SF4/离子/溶剂）即使碰巧有 N/CA/C/O，也会因为 record_type=HETATM 被剔除
                # - “蛋白组分但不在 onehot 支持表里的残基”允许保留，后续会走 'X' 兜底（onehot 第104维）
                ...
                if len(c_alpha) == 1 and len(n) == 1 and len(c) == 1 and len(o) == 1:
                    ...
                    tip = res_atoms.select_atoms(
                        f"name {aa2tip[three2idx[three2self.get(res_name, 'X')]]} and record_type ATOM"
                    )
                    if len(tip) == 0:
            ...
            if lm_embedding_chains is not None:
                assert lm_embedding_chain is not None
        ...
        flat_residues = [residue for chain in pure_res_lis for residue in chain]
        X_center_of_mass_np = np.concatenate(
            [
                residue.atoms.select_atoms("not type H").center_of_mass(
                    compound="residues"
                )
                for residue in flat_residues
            ]
        )
        side_chain_mass_np = np.asarray(
            [
                (
                    sidechain_atoms.center_of_mass()
                    if len(sidechain_atoms) > 0
                    else residue.atoms.select_atoms("name CA").positions[0]
                )
                for residue in flat_residues
                for sidechain_atoms in [
                    residue.atoms.select_atoms("not name C N CA O and not type H")
                ]
            ]
        )
        X_center_of_mass = torch.from_numpy(X_center_of_mass_np)  # [n_residues, 3]
        side_chain_mass = torch.from_numpy(side_chain_mass_np)  # [n_residues, 3]
        edge_index = torch_cluster.knn_graph(c_alpha_coords, k=top_k)
        edge_pairs = edge_index.T.detach().cpu().numpy()
        dis_minmax = torch.from_numpy(
            np.asarray(
                [
                    obatin_edge(flat_residues, int(src), int(dst))
        ...
        cedist = (
            _pairwise_row_distance(X_center_of_mass_np, edge_index) * 0.1
        ).view(
            -1, 1
        )  # distance between two centers of mass # [n_edges, 1]
        csdist = (
            _pairwise_row_distance(side_chain_mass_np, edge_index) * 0.1
        ).view(
            -1, 1
        )  # distance between two centers of sidechain mass # [n_edges, 1]
        edge_connect = torch.from_numpy(
            (
                (np.abs(edge_pairs[:, 0] - edge_pairs[:, 1]) == 1)
                & np.asarray(
                    [flat_residues[src].segid == flat_residues[dst].segid for src, dst in edge_pairs]
                )
            ).astype(np.int64)
        ).view(
            -1, 1
