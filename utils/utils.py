...
def get_model(args, confidence_mode=False, no_parallel=False):
    """根据模式返回得分/置信模型，并按GPU数量包DataParallel"""
    if confidence_mode:
...
def get_optimizer_and_scheduler(args, model, scheduler_mode="min"):
    ...
    支持：
      - plateau（默认老逻辑）
      - cosine（配 warmup_steps + total_steps，cosine decay）
    """
    ...
    if args.scheduler == "plateau":
        ...
        if not total_steps:
            print("No total_steps provided, fallback to None scheduler")
            scheduler = None
        else:
            import math
